                     ...
                     <div class="widget-header">
                        <div class="row">
                           <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                              <h4>Recipient Name : &nbsp;<span class="text-danger"><strong><i>{{ $recipientName }}</i></strong></span></h4>
                           </div>
                        </div>
                     </div>
                           ...
                           <div class="tab-pane fade show active" id="animated-underline-home" role="tabpanel" aria-labelledby="animated-underline-home-tab">
                              ...
                              <table id="zero-config" class="table table-striped" style="width:100%">
                                 ...
                                 <tbody>
                                    ...
                                    <tr>
                                       <td><strong>{{ $dashboard->dashboard_name }}</strong></td>
                                       <td><strong><span class="text-muted">{{ $dashboard->created_time}}</span></strong></td>
                                       <td>
                                          <label class="switch s-icons s-outline s-outline-primary mt-4 mr-2">
                                             <input type="checkbox" class="row-checkbox" data-dashboard-id="{{ $dashboard->dashboards_id }}" data-recipients-id="{{ $dashboard->recipients_id }}" {{ $dashboard->is_active === 'yes' ? 'checked' : '' }}>
   ...
   <script>
      $(document).ready(function() {
         $('.row-checkbox').on('change', function() {
            ...
            const dashboardId = rowCheckbox.data('dashboard-id');
            const recipientsId = rowCheckbox.data('recipients-id');
            const newIsActive = rowCheckbox.prop('checked') ? 'yes' : 'no';
            ...
            Swal.fire({
               ...
               // Perbarui URL dengan dashboardId dan recipientsId
               const url = "{{ route('update-status', ['dashboardId' => ':dashboardId', 'recipientsId' => ':recipientsId']) }}"
                  .replace(':dashboardId', dashboardId)
                  .replace(':recipientsId', recipientsId);
               ...
               $.ajax({
                  ...
                  success: function(response) {
                     Swal.fire({
                        ...
                        timer: 1500,
                        onClose: () => {
                           rowCheckbox.prop('checked', newIsActive === 'yes');
