  ...
  <script>
      $(document).ready(function() {
          ...
          function updateCards(data) {
              ...
              // Bersihkan konten pada div dengan id dashboardContainer
          ...
          <span class="mt-4">
              <strong>
                  <h4>Oops data yang anda cari tidak ditemukan.</h4>
              </strong>
          </span>
      `;
                  dashboardContainer.append(noDataHtml);
              } else {
                  // Perbarui card dengan data yang diterima dari server
                  $.each(data, function(index, row) {
                      var dashboardNameSlug = encodeURIComponent(row.dashboard_name.replace(/\s+/g, '-'));
                      var detailButton = '';

                      if (row.dashboard_status === 0) {
                          // Jika dashboard status adalah 0, tampilkan tombol "Under Maintenance"
                          detailButton = `<a href="#" class="btn btn-danger mt-2 btn-under-maintenance">Under Maintenance</a>`;
                      } else if (row.is_allowed) {
                          // Jika dashboard diizinkan untuk pengguna, tampilkan tombol Detail Dashboard
                          detailButton = `<a href="/portaldashboard/detail/${dashboardNameSlug}" class="btn btn-relief-primary mt-2">Detail Dashboard</a>`;
                      } else {
                          // Jika dashboard tidak diizinkan untuk pengguna, tampilkan tombol tidak aktif dan tombol Add Request
                          detailButton = `
        <div class="not-allowed">
            <button class="btn btn-primary mt-2" disabled>Detail Dashboard</button>
            <button class="btn btn-success mt-2" data-bs-toggle="modal" data-bs-target="#editCategoryModal${row.dashboard_id}">Add Request</button>
        </div>
    `;
                      }
